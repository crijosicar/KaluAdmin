...
class WalletController extends Controller {
    ...
    public function getIncomesAndExpensesByUser(Request $request) {
        ...
        $results = DB::select("SELECT LCASE(lvl.valor) AS categoria,
                              ...
                              GROUP BY lvl.valor", ['tipo_transaccion_id' => $tipoTransaccion->id,
    ...
    public function getTimeframeIncomesAndExpensesByUser(Request $request) {
        ...
        $niceNames = array(
            'user_id' => 'ID de usuario',
            'token' => 'token',
            'categoria_activo' => 'categoria de activo'
        );

        $validator = Validator::make($request->all(), [
                        'user_id' => 'required',
                        'token' => 'required',
                        'categoria_activo' => 'required'
                      ], $messages, $niceNames
                    );
        ...
        if(count($results) && count($results) > 1){
          ...
          return response()->json([
                      ...
                      'messages' => array(
                        "unit" => "N/A",
                        "value" => "No tienes suficientes registros"
                      )
          ]);
    ...
    public function getIncomesAndExpensesExpectedByUser(Request $request) {
        ...
        $comidaList = array_filter($results, function ($element) {
            return ($element->categoria == "COMIDA");
        });
        $ropaList = array_filter($results, function ($element) {
            return ($element->categoria == "ROPA");
        });
        $facturasList = array_filter($results, function ($element) {
            return ($element->categoria == "FACTURAS");
        });
        $comunicacionesList = array_filter($results, function ($element) {
            return ($element->categoria == "COMUNICACIONES");
        });
        $entretenimientoList = array_filter($results, function ($element) {
            return ($element->categoria == "ENTRETENIMIENTO");
        });
        $saludList = array_filter($results, function ($element) {
            return ($element->categoria == "SALUD");
        });
        $hogarList = array_filter($results, function ($element) {
            return ($element->categoria == "HOGAR");
        });
        $transporteList = array_filter($results, function ($element) {
            return ($element->categoria == "TRANSPORTE");
        });
        ...
        $depositoList = array_filter($results, function ($element) {
            return ($element->categoria == "DEPOSITOS");
        });
        $salarioList = array_filter($results, function ($element) {
            return ($element->categoria == "SALARIO");
        });
        $ahorrosList = array_filter($results, function ($element) {
            return ($element->categoria == "AHORROS");
        });
    ...
    public function getDetailsCategoryByUser(Request $request) {
        ...
        $categoriaActivo = ListaValor::where("categoria", "CATEGORIA_ACTIVO")
                ->where("valor", $request->input('categoria'))
                ->first();
        ...
        $results = DB::select("SELECT
                                    demv.nombre_activo AS activo,
                                    demv.monto
                               FROM detalle_movimiento AS demv
                                INNER JOIN movimientos AS movs ON (demv.movimiento_id = movs.id)
                                INNER JOIN user_movimientos AS umvs ON(movs.id = umvs.movimiento_id)
                               WHERE demv.categoria_activo_id = :categoria_activo_id
                                  AND umvs.user_id = :user_id
                                  AND umvs.created_at >= :min_date
                                  AND umvs.created_at <= :max_date",
                ['categoria_activo_id' => $categoriaActivo->id,
                    'user_id' => $request->input('user_id'),
                    'min_date' => $minDate,
                    'max_date' => $maxDate
